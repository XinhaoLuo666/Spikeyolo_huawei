...
# export RANK_ID=0,1,2,3,4,5,6,7  #可能不需要添加此环境变量
# netstat -anp|grep 8118 用这个锁定并行化运行后的僵尸进程，否则两个并行化进程无法一起运行
# msrun --worker_num=8 --local_worker_num=8 --bind_core=True --log_dir=./yolov8_log python train.py --config ./configs/yolov8/yolov8n.yaml  --is_parallel True --run_eval True
...
def get_parser_train(parents=None):
    ...
    parser.add_argument("--run_eval", type=ast.literal_eval, default=True, help="Whether to run eval during training")
    ...
    parser.add_argument("--data_dir", type=str, default="/home/mseco/dataset/COCO/",
...
def train(args):
    ...
    print("============================args.run_eval:",args.run_eval)
    print("============================args.ast.literal_eval:",args.is_parallel)
    ...
    if not args.ms_datasink:
        print("1===========================================ms:", ms.get_context("mode"))
        trainer.train(
            epochs=args.epochs,
