...
class mem_update(nn.Cell):
    ...
    def construct(self, x): #前向传播
        ...
        spike = ops.zeros(x[0].shape)
        output = ops.zeros(x.shape)
        mem = 0
        time_window = x.shape[0]
        for i in range(time_window):
            if i >= 1:
                mem = (mem - spike.detach()) * decay + x[i]
...
class RepConv(nn.Cell):
    def __init__(
        ...
        conv1x1 = nn.Conv2d(in_channel, in_channel, 1,  has_bias=False, group=group)
        bn = BNAndPadLayer(pad_pixels=padding, num_features=in_channel)
        conv3x3 = nn.SequentialCell(
            # mem_update(), #11111
            nn.Conv2d(in_channel, in_channel, kernel_size, group=in_channel, has_bias=False),  #这里也是分组卷积
            # mem_update(),  #11111
            nn.Conv2d(in_channel, out_channel, 1,   group=group, has_bias=False),
...
class SepRepConv(nn.Cell):  # 放在Sepconv最后一个1*1卷积，采用3*3分组+1*1降维的方式实现，能提0.5个点。之后可以试试改成1*1降维和3*3分组
    def __init__(
            self,
            in_channel,
            out_channel,
        ...
        padding = int((kernel_size - 1) / 2)
        # hidden_channel = in_channel
        #         conv1x1 = nn.Conv2d(in_channel, in_channel, 1, 1, 0, has_bias=False, group=group)
        bn = BNAndPadLayer(pad_pixels=padding, num_features=in_channel)
        conv3x3 = nn.SequentialCell(
            nn.Conv2d(in_channel, in_channel, kernel_size,  group=in_channel, has_bias=False),  # 这里也是分组卷积
            mem_update(),  # 11111
...
class SepConv(nn.Cell):
    ...
    def __init__(self,
        ...
        self.dwconv2 = nn.Conv2d(
            ...
            padding=padding, group=med_channels,pad_mode="pad")  # depthwise conv
        #         self.pwconv3 = nn.Conv2d(med_channels, dim, kernel_size=1, stride=1, has_bias=has_bias,group=1)
    ...
    def construct(self, x):
        ...
        x = self.bn1(self.pwconv1(ops.flatten(x, start_dim=0, end_dim=1))).reshape(T, B, -1, H, W)  # flatten：从第0维开始，展开到第一维
        x = self.lif2(x)
        x = self.bn2(self.dwconv2(ops.flatten(x, start_dim=0, end_dim=1))).reshape(T, B, -1, H, W)
        x = self.lif3(x)
        x = self.bn3(self.pwconv3(ops.flatten(x, start_dim=0, end_dim=1))).reshape(T, B, -1, H, W)
        return x
...
class MS_GetT(nn.Cell):
    ...
    def construct(self, x):
        ...
        x = (x.unsqueeze(0)).repeat(self.T,axis=1)
        return x
...
class SpikeDetect(nn.Cell):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        for x in ch:
            self.cv2.append(SpikeConv(x, c2, 3))
            self.cv2.append(SpikeConv(c2, c2, 3))
            self.cv2.append(SpikeConvWithoutBN(c2, 4 * self.reg_max, 1))

        self.cv3 = nn.SequentialCell()
        for x in ch:
            self.cv3.append(SpikeConv(x, c3, 3))
            self.cv3.append(SpikeConv(c3, c3, 3))
            self.cv3.append(SpikeConvWithoutBN(c3, self.nc, 1))
        # self.cv2 = nn.SequentialCell(
